...
async def letter_spam(cspammer):
...
async def spammers(spammer):
    counter = int(spammer.pattern_match.group(1).split(" ", 1)[0])
    spam_message = str(spammer.pattern_match.group(1).split(" ", 1)[1])
    await spammer.delete()
